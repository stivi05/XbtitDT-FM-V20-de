...
std::string Cserver::insert_peer(const Ctracker_input& v, bool udp, t_user* user)
{
	...
	if (user && !user->can_announce)
		return "access denied";
	BOOST_FOREACH(t_deny_from_clients::const_reference i, m_deny_from_clients)
	{
		if (boost::starts_with(v.m_peer_id, i))
			return bts_banned_client;
	}
	t_file& file = m_files[v.m_info_hash];
	if (!file.ctime)
		file.ctime = time();
	if (v.m_left && user && user->wait_time && file.download_multiplier && file.ctime + user->wait_time > time())
	...
	if (m_use_sql && user && file.fid)
	{
			...
			&& v.m_uploaded >= i->second.uploaded)
		{
			...
			if (time() > i->second.mtime) 		 
			   { 		 
				   down_rate = downloaded / (time() - i->second.mtime); 		 
				   up_rate = uploaded / (time() - i->second.mtime); 		 
			   }
		}
		m_files_users_updates_buffer += Csql_query(m_database, "(?,1,?,?,?,?,?,?,?,?,?,?,?),")
			...
			.p(v.m_left)
			.p(uploaded)
			.p(v.m_event == Ctracker_input::e_completed ? time() : -1)
			.p(down_rate) 		 
	        .p(up_rate) 		 
	        .p(v.m_peer_id)			
			.p(time())
		...
		{
			m_users_updates_buffer += Csql_query(m_database, "(?,?,?),")
				.p(downloaded * file.download_multiplier/100)
				.p(uploaded * file.upload_multiplier/100)
				.p(user->uid)
				.read();
		}
	}
...
void Cserver::t_file::clean_up(time_t t, Cserver& server)
{
	for (t_peers::iterator i = peers.begin(); i != peers.end(); )
	{
		if (i->second.mtime < t)
		{
			...
			if (i->second.uid)
				server.m_files_users_updates_buffer += Csql_query(server.m_database, "(0,0,0,0,-1,0,-1,-1,-1,-1,-1,?,?),").p(fid).p(i->second.uid).read();
			peers.erase(i++);
			dirty = true;
		}
		else
...
void Cserver::read_db_deny_from_hosts()
{
	...
	try
	{
		Csql_result result = Csql_query(m_database, "select peer_id from xbt_deny_from_clients").execute();
...
void Cserver::read_db_files_sql()
{
	try
	{
		...
		{
			Csql_result result = Csql_query(m_database, "select info_hash, ?, down_multi, up_multi from ? where flags & 2").p_name(column_name(column_files_fid)).p_name(table_name(table_files)).execute();
			for (Csql_row row; row = result.fetch_row(); )
			{
				if (row[0].size() != 20)
					continue;
				...
				if (i != m_files.end())
				{
					i->second.download_multiplier = row[2].i();
					i->second.upload_multiplier = row[3].i();
				}
				Csql_query(m_database, "update ? set flags = flags & ~2 where ? = ?").p_name(table_name(table_files)).p_name(column_name(column_files_fid)).p(row[1].i()).execute();
			}
		}
		if (m_files.empty())
			...
			return;
		Csql_result result = Csql_query(m_database, "select info_hash, ?, ?, ctime, down_multi, up_multi from ? where ? >= ?")
		...
		for (Csql_row row; row = result.fetch_row(); )
		{
			...
			file.fid = row[2].i();
			file.ctime = row[3].i();
			file.download_multiplier = row[4].i();
			file.upload_multiplier = row[5].i();
		}
	}
	catch (Cdatabase::exception&)
	{
...
void Cserver::read_db_users()
{
	...
	try
	{
		Csql_query q(m_database, "select ?, can_announce");
...
void Cserver::write_db_users()
{
	...
	if (!m_files_users_updates_buffer.empty())
	{
		...
		try
		{
			m_database.query("insert into " + table_name(table_files_users) + " (active, announced, completed, downloaded, `left`, uploaded, completed_time, down_rate, up_rate, peer_id, mtime, fid, uid) values "
				...
				+ "  completed_time = if(values(completed_time) = -1, completed_time, values(completed_time)),"
				+ "  down_rate = if(values(down_rate) = -1, down_rate, values(down_rate)),"  		 
	            + "  up_rate = if(values(up_rate) = -1, up_rate, values(up_rate)),"  		 
	            + "  peer_id = if(values(peer_id) = -1, peer_id, values(peer_id)),"
