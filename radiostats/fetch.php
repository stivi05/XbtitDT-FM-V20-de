      ...
      if (mysqli_num_rows($Query)>0)
      {
          ...
          while($List=mysqli_fetch_assoc($Query))
          {
              if (preg_match ('@' . $todayis . '@Ui', $List['activedays']))
              {
                  ...
                  {
                      $found=1;
                      echo "<tr><td class=header><b>".$language['BL_DJ'].":</b></td><td class=lista>&nbsp;<a href=index.php?page=userdetails&id=".$List["realid"]."><b>".unesc($List["prefixcolor"]).$List["username"].unesc($List["suffixcolor"])."</b></a></td></tr>";
                  }
		      }
		  }
		  if($found==0)
		  {
              $djname=mysqli_real_escape_string($DBDT,$shoutcast->GetServerTitle());
              ...
              if(@mysqli_num_rows($petr1)>0)
              {
                  ...
                  $djname="<a href='index.php?page=userdetails&id=".$fied["id"]."'>".unesc($fied["prefixcolor"] . $djname . $fied["suffixcolor"])."</a>";
              }
              else
                  $djname=unesc($djname);
