...
class XbtitBtchat {
		var $maxTorrents = '50';
        ###     
        function getFeedbtchat($feed_url) {
				$cpt=0;
                ...
                if (isset($_POST['dump'])) {
                        $this->updateBtchat($feed_url,$_POST['category'],$_POST['uploaderid']);
                }
        ...
        ###     
        function updateBtchat($feed_url,$cat,$userid) {
				global $TORRENTSDIR;
				$cpt=0;
                $content = file_get_contents($feed_url);        
                $x = new SimpleXmlElement($content); 
                ...
                foreach($x->channel->item as $entry) { 
                                if(!$this->checkExistetorrent($entry->torrent->infoHash)) {
                                         $this->DownloadTorrent($entry->enclosure->attributes(),$entry->torrent->infoHash);
                                         $this->insertTorrent(
		            ...
		            $cpt++;
					if($cpt==$this->maxTorrents) break; //on sort de la boucle
                	}        
        }
        ###
		 function DownloadTorrent($a,$name) {
				      		global $TORRENTSDIR;
				            $fp = fopen (dirname(__FILE__).'/../../'.$TORRENTSDIR.'/'.strtolower($name).'.btf', 'w+');
				            $ch = curl_init($a);	 
                        	...
                        	curl_setopt($ch, CURLOPT_REFERER, 'http://www.bt-chat.com/');				            
				            curl_exec($ch);
				            curl_close($ch);
				            fclose($fp); 
			}
