...
class XbtitThepiratebay {
		var $maxTorrents = '50';
        ###     
        function getFeedthepiratebay($feed_url) {
				$cpt=0;
                ...
                foreach($x->channel->item as $entry) {
		           ...
		           // $h = $this->exploserChaine($entry->guid); 
		            
                if (isset($_POST['dump'])) {
                        $this->updatePiratebay($feed_url,$_POST['category'],$_POST['uploaderid']);
                }
                if(!$this->checkExistetorrent($entry->torrent->infoHash)) {
                        $r .= "
                        <li><a href='$entry->comments' title='$entry->title'>" . $entry->title . "</a></li>
                        <li>Hash: " . $entry->torrent->infoHash . " <br /></li>
                        <li>Date: " . $entry->pubDate . " <br /></li>
                        <li>Taille: " . $entry->torrent->contentLength . " <br /></li>
                        <li>
                         https://torrents.thepiratebay.gd/" . $h[4] . "/". $entry->title .".torrent
                        </li>
                        <br /><hr>";
                } else {
                        return false;
        ...
        ###     
        function updatePiratebay($feed_url,$cat,$userid) {
				global $TORRENTSDIR;
                $content = file_get_contents($feed_url);        
                $x = new SimpleXmlElement($content); 
                ...
                foreach($x->channel->item as $entry) { 
		            ...
		            $h = $this->exploserChaine($entry->guid); 
                                ...
                                if(!$this->checkExistetorrent($entry->torrent->infoHash)) {
                                         $this->DownloadTorrent('https://www.thepiratebay.gd/'.$h[4].'/'.$entry->title.'.torrent',$entry->torrent->infoHash);
                                         $this->insertTorrent(
                                         		dirname(__FILE__).'/../../'.$TORRENTSDIR.'/'.strtolower($entry->torrent->infoHash).'.btf',
                                         		$entry->torrent->infoHash,
                                         		$entry->title,
                                         		$userid,
                                         		$cat
                                         ); 
        ...
        function DownloadTorrent($a,$h) {
                global $TORRENTSDIR;
                        ...
                        curl_setopt($ch, CURLOPT_REFERER, 'https://www.thepiratebay.gd/');
                        curl_exec($ch);
                        curl_close($ch);
                        fclose($fp);
